...
import { invoicesAPI, warehousesAPI } from '../api';
import { Button } from './ui/button';
import { Input } from './ui/input';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from './ui/dialog';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from './ui/select';
import { Tabs, TabsContent, TabsList, TabsTrigger } from './ui/tabs';
import { Plus, FileText, Upload, Image as ImageIcon, FileUp, Eye, Package } from 'lucide-react';
...
export const InvoicesPage = () => {
  ...
  const loadData = async () => {
    try { const [iR, wR] = await Promise.all([invoicesAPI.getAll(), warehousesAPI.getAll()]); setInvoices(iR.data); setWarehouses(wR.data); }
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      setFormData({ invoice_number: d.invoice_number || '', supplier_name: d.supplier_name || '', issue_date: d.issue_date || '', total_value: d.total_value || 0, tax_value: d.tax_value || 0,
  ...
  const handleImageUpload = async (e) => {
    ...
    reader.onloadend = async () => {
      try {
        ...
        setFormData({ invoice_number: res.data.invoice_number || '', supplier_name: res.data.supplier_name || '', issue_date: res.data.issue_date || '', total_value: res.data.total_value || 0, tax_value: res.data.tax_value || 0,
  ...
  const handleProcessItems = async () => {
    if (!selectedWarehouse) { toast.error('Selecione um deposito'); return; }
    try {
      const res = await invoicesAPI.processItems(selectedInvoice.id, selectedWarehouse);
      toast.success(res.data.message); setProcessOpen(false); loadData();
    } catch (err) { toast.error(err.response?.data?.detail || 'Erro ao processar'); }
  };

  const handleStatusChange = async (invoiceId, status) => {
    try {
      // We'll use a direct API call since we don't have a specific update endpoint
      toast.info(`Status da nota ${invoiceId.substring(0,8)} seria alterado para ${status}`);
    } catch {}
  };
    ...
    <div className="p-4 md:p-8" data-testid="invoices-page">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">Upload PDF/XML ou escaneie imagens com IA</p>
        </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="file" className="space-y-4">
                <div className="border-2 border-dashed border-zinc-300 rounded-lg p-8 text-center">
                  ...
                  <p className="text-xs text-zinc-500 mb-4">XML sera parseado. PDF sera processado com IA.</p>
              ...
              <TabsContent value="ocr" className="space-y-4">
                <div className="border-2 border-dashed border-zinc-300 rounded-lg p-8 text-center">
                  ...
                  <p className="text-sm text-zinc-600 mb-4">Upload de imagem para extracao automatica com IA</p>
                    ...
                    {formData.items.length > 0 && (
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          <thead className="bg-zinc-50"><tr><th className="px-3 py-2 text-left text-xs font-semibold text-zinc-500">Produto</th><th className="px-3 py-2 text-right text-xs font-semibold text-zinc-500">Unidades</th><th className="px-3 py-2 text-right text-xs font-semibold text-zinc-500">Valor Unit.</th><th className="px-3 py-2 text-right text-xs font-semibold text-zinc-500">Total</th></tr></thead>
                          <tbody className="divide-y divide-zinc-100">
                            {formData.items.map((it, i) => (
                              <tr key={i}><td className="px-3 py-2 text-zinc-900">{it.product_name}</td><td className="px-3 py-2 text-right font-mono">{it.quantity}</td><td className="px-3 py-2 text-right font-mono">R$ {it.unit_price.toFixed(2)}</td><td className="px-3 py-2 text-right font-mono">R$ {it.total.toFixed(2)}</td></tr>
                            ))}
                          </tbody>
                        </table>
                      </div>
      ...
      <Dialog open={processOpen} onOpenChange={setProcessOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Processar Itens da Nota</DialogTitle></DialogHeader>
          <p className="text-sm text-zinc-600 mb-4">Os itens serao criados como produtos e adicionados ao estoque do deposito selecionado.</p>
          <Select value={selectedWarehouse} onValueChange={setSelectedWarehouse}>
            <SelectTrigger><SelectValue placeholder="Selecione o deposito" /></SelectTrigger>
            <SelectContent>{warehouses.map(w => <SelectItem key={w.id} value={w.id}>{w.name}</SelectItem>)}</SelectContent>
          </Select>
          <Button onClick={handleProcessItems} className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white">Processar Itens</Button>
        </DialogContent>
      </Dialog>
          ...
          {selectedInvoice && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 p-4 bg-zinc-50 rounded-lg">
                ...
                <div><p className="text-xs text-zinc-500">Valor Total</p><p className="text-sm font-medium font-mono">R$ {selectedInvoice.total_value.toFixed(2)}</p></div>
                <div><p className="text-xs text-zinc-500">Impostos</p><p className="text-sm font-medium font-mono">R$ {(selectedInvoice.tax_value || 0).toFixed(2)}</p></div>
              </div>
              {(selectedInvoice.items || []).length > 0 && (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-zinc-50"><tr><th className="px-3 py-2 text-left text-xs font-semibold text-zinc-500">Produto</th><th className="px-3 py-2 text-right text-xs font-semibold text-zinc-500">Unidades</th><th className="px-3 py-2 text-right text-xs font-semibold text-zinc-500">Valor Unit.</th><th className="px-3 py-2 text-right text-xs font-semibold text-zinc-500">Total</th></tr></thead>
                    <tbody className="divide-y divide-zinc-100">
                      {selectedInvoice.items.map((it, i) => <tr key={i}><td className="px-3 py-2">{it.product_name}</td><td className="px-3 py-2 text-right font-mono">{it.quantity}</td><td className="px-3 py-2 text-right font-mono">R$ {it.unit_price.toFixed(2)}</td><td className="px-3 py-2 text-right font-mono">R$ {it.total.toFixed(2)}</td></tr>)}
                    </tbody>
                  </table>
                </div>
              )}
      ...
      <div className="bg-white rounded-xl border border-zinc-200 shadow-sm overflow-x-auto">
        <table className="w-full min-w-[700px]" data-testid="invoices-table">
            ...
            {invoices.map(inv => (
              <tr key={inv.id} className="hover:bg-zinc-50 transition-colors">
                ...
                <td className="px-4 py-3 text-center">
                  <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${inv.status === 'processed' ? 'bg-green-100 text-green-700' : inv.status === 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    {inv.status === 'pending' && <button onClick={() => { setSelectedInvoice(inv); setProcessOpen(true); }} title="Processar itens" className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg"><Package className="h-4 w-4" /></button>}
                  </div>
                </td>
              </tr>
            ))}
